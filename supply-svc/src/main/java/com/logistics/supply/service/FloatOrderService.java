...
public class FloatOrderService {
    ...
    public Page<FloatOrder> findPendingGrnApprovalFromStoreManager(Pageable pageable, int departmentId) {
        ...
        Page<FloatOrder> orders = floatOrderRepository
                .findFloatOrderPendingGrnApprovalFromStoreManager(pageable, departmentId);
        orders.forEach(order -> Hibernate.initialize(order.getSupportingDocument()));
        return orders;
    }
    ...
    public Page<FloatOrder> findByEmployee(int employeeId, Pageable pageable) {
        ...
        Page<FloatOrder> orders = floatOrderRepository.findByCreatedByIdOrderByIdDesc(employeeId, pageable);
        orders.forEach(order -> Hibernate.initialize(order.getSupportingDocument()));
        return orders;
    }
    ...
    public Page<FloatOrder> findFloatOrderToClose(int pageNo, int pageSize) {
        ...
        Pageable pageable = PageRequest.of(pageNo, pageSize, Sort.by("id").descending());
        Page<FloatOrder> orders = floatOrderRepository.findAll(specification, pageable);
        orders.forEach(order -> Hibernate.initialize(order.getSupportingDocument()));
        return orders;
    }
    ...
    public Page<FloatOrder> findAllFloatOrder(int pageNo, int pageSize) {
        ...
        Pageable pageable = PageRequest.of(pageNo, pageSize, Sort.by("id").descending());
        Page<FloatOrder> orders = floatOrderRepository.findAll(pageable);
        orders.forEach(order -> Hibernate.initialize(order.getSupportingDocument()));
        return orders;
    }
